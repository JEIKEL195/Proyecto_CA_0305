...
def jugar_blackjack_como_casa(monto):
    ...
    # La siguiente función no se utiliza.
    ...
    while balance > 0:
        ...
        for i, mano in enumerate(hands):
            # El jugador juega igual que el dealer: pide hasta tener más de 16
            while total(mano) <= 16:
                dealCard(mano, deck)
...
# Iniciar el juego
print(jugar_blackjack_como_casa(10))


inicio = time.time()
montos = list(range(10, 110, 10))  # [10, 20, ..., 100]
jugadas = [jugar_blackjack_como_casa(monto) for monto in montos]


plt.plot(montos, jugadas, marker='o', linestyle='-', color ='green')
plt.xlabel('Monto inicial')
plt.ylabel('Número de jugadas hasta perder')
plt.title('Relación entre monto inicial y jugadas\n Estrategia: Jugar como Casa')
plt.grid(True)
plt.show()
...
def promedio_jugadas(monto, repeticiones=100):
    ...
    """
    resultados = [jugar_blackjack_como_casa(monto) for _ in range(repeticiones)]
    return np.mean(resultados)

jugadas_promedio = [promedio_jugadas(monto) for monto in montos]

plt.plot(montos, jugadas_promedio, marker='o', linestyle='-', color = 'green')
plt.xlabel('Monto inicial')
...
plt.errorbar(montos, promedios, yerr=desviaciones, fmt='-o', capsize=5, color = 'Green')
plt.xlabel('Monto inicial')
plt.ylabel('Jugadas hasta perder (promedio ± std)')
plt.title('Relación entre monto inicial y jugadas\nEstrategia: Jugar como Casa con barras de error\nPromedio de 100 simulaciones')
plt.grid(True)
plt.show()
...
inicio = time.time()
monto = 100
jugadas = [jugar_blackjack_como_casa(monto) for _ in range(1000)]

plt.hist(jugadas, bins=30, color='green', edgecolor='black')
...
for monto in montos:
    for _ in range(100):
        jugadas = jugar_blackjack_como_casa(monto)
        data.append({'Monto': monto, 'Jugadas': jugadas})

df = pd.DataFrame(data)
sns.boxplot(x='Monto', y='Jugadas', data=df, color = 'green')
...
for monto in montos:
    cuenta = sum([jugar_blackjack_como_casa(monto) > rondas for _ in range(1000)])
    prob = cuenta / 1000
    sobrevive.append(prob)

plt.plot(montos, sobrevive, marker='o', color = 'green')
plt.xlabel('Monto inicial')
...
for rondas in rondas_lista:
    cuenta = sum([jugar_blackjack_como_casa(monto_fijo) > rondas for _ in range(1000)])
    prob = cuenta / 1000
    probabilidades.append(prob)

plt.plot(rondas_lista, probabilidades, marker='o', color = 'green')
