...
def jugar_blackjack():
    ...
    # Inicializa el balance del jugador con 1000, este valor es solo por ser interactivo.
    balance = 1000
    print(f"Tu balance inicial es: ${balance}")
    ...
    while balance > 0:
        ...
        while True:
            try:
                bet = int(input("¿Cuánto deseas apostar? (0 para salir): "))
        ...
        if dealerHand[0] == 'A' and len(dealerHand) == 2:
            ...
            while True:
                opcion = input("¿Quieres tomar el seguro? (s/n): ").lower()
                time.sleep(1)
                if opcion == 's':
        ...
        if puede_split:
            eleccion = input(f"Tienes {playerHand}. ¿Deseas dividir (s/n)? ").lower()
            time.sleep(1)
